...
23. **Uvicorn HTTP Upgrade headers (2026-08-10)**: Reforger's Enforce `RestContext` sends `Connection: Upgrade` headers on normal HTTP REST requests. Uvicorn's HTTP protocol layer (h11/httptools) logs `WARNING: Unsupported upgrade request.` for EVERY such request. The `ws="none"` uvicorn parameter does NOT fix this — it only disables the WebSocket protocol, but the HTTP handler still warns. **Root cause fix**: monkey-patch `H11Protocol._should_upgrade` and `HttpToolsProtocol._should_upgrade` at module level in `main.py` to return `False` silently for non-WebSocket upgrades. This is CORRECT HTTP behavior (RFC 7230 §6.1 allows Upgrade headers) and works in both CLI and `python main.py` modes. Also: `start_bridge.bat` must use `python main.py` (not `python -m uvicorn main:app`) so all uvicorn parameters (timeout_keep_alive, limit_concurrency, ws="none") take effect.
