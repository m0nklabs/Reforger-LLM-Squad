...
### F2.5: DS mod loading via BI Workshop — ✅ DONE (2026-08-09)

Mod published to BI Workshop (unlisted). `game.mods[]` with addon GUID as `modId`.
DS downloads mod → 5637 files → scripts compile + execute on DS.

### F2.6: MP slave group + auto-follow — 🔄 WIP (2026-08-09)

**Problem**: AI soldiers spawn on DS but don't appear in player's group UI and can't be commanded.
**Root cause**: `AddAgent()` has no RPL broadcast. Vanilla uses master/slave group architecture.
**Fix**: Create slave group via `EnsureSlaveGroup()`, add AI via `slaveGroup.AddAgentFromControlledEntity()`.
**Fix**: Combat component crash — delay `ActivateAI()` by 500ms after group assignment.
**Auto-follow**: Follow waypoint created automatically after spawn.

### F2.7: Individual AI brains (vision) — 🔲 TODO

**Goal**: Each AI squad member has its own personality, refreshed periodically by the LLM.

**Concept**:
- Each AI has a `personality` (aggressive, cautious, joker, veteran, rookie)
- Every 30-60s, the LLM generates a "thought" for each AI based on the current situation
- Thoughts can range from tactical ("I see movement at 200m northeast") to personal ("I'm bored, I could shoot some carrots")
- When the situation is quiet, AI members may start chatting with each other:
  - "Hey, you see that bird?"
  - "Nah, I'm watching the treeline. Last time I wasn't watching, we got ambushed."
  - "You always say that. Relax."
- These conversations are generated by the LLM and displayed as subtitles/chat messages
- Eventually: TTS for each AI with a distinct voice

**Implementation sketch**:
```
LLMBridge.Update() → every 30s → for each AI member:
  POST /ai_thought {ai_id, personality, situation, recent_events}
  → LLM returns {thought, mood, action_hint}
  → Display as subtitle/chat in game
  → If two AIs are both "bored", trigger conversation between them
```
...
| F2.1 | WorldState reporter | ✅ DONE | 2 |
| F2.2 | LLM tactical router | ✅ DONE | 2 |
| F2.3 | Waypoint execution | ✅ DONE | 2 |
| F2.4 | Rank + faction isolation | 🔲 TODO | 2 |
| F2.5 | DS mod loading (Workshop) | ✅ DONE | 2 |
| F2.6 | MP slave group + auto-follow | 🔄 WIP | 2 |
| F2.7 | Individual AI brains | 🔲 TODO | 2 |
