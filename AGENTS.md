...
| Task | Exact action |
|---|---|
| Start bridge | `start_bridge.bat` (port 5001, auto-elevates admin for voice PTT key). Uses `cmd /c` — console window CLOSES when the bridge is killed (no zombie windows) |
...
- **LLM**: Ollama-compatible proxy `http://192.168.1.35:11434/v1`, model `llama3` (8B — switched from llama3.2-3b in D.2)
...
60. **Bridge console must auto-close (FIXED)**: ALWAYS start the bridge with `cmd /c` (NOT `cmd /k`) — the console window closes when the python process ends, including when it is taskkilled. `start_bridge.bat` line: `start "Reforger-LLM-Bridge" /min cmd /c "cd /d %~dp0\python_bridge && venv\Scripts\python.exe main.py"`. `scripts/restart_bridge_console.ps1` uses the same pattern. NEVER introduce a `pause` or `cmd /k` in bridge launchers — zombie console windows pile up after repeated kills.
61. **CLIENT must not run the bridge client (FIXED — CRITICAL)**: Without a `Replication.IsServer()` guard in `SCR_BaseGameMode.OnGameStart`, the game CLIENT also instantiates LLMBridge — it polls /orders + /ai_thought against the SAME bridge, STEALS orders from the DS (client logs "No AI group found" while the DS-side order never executes), and sends duplicate SITREPs. Symptom: dashboard buttons do nothing even though "Live order received" appears in logs. Fix: guard OnGameStart with `if (!Replication.IsServer()) return;` — bridge client is DS-only. ALWAYS check both DS log (logs_<DS start>) and client log (logs_<client start>) — they share the logs directory!
62. **LLM HOLD wiped CO waypoints (FIXED — CRITICAL)**: Every 30s SITREP cycle the LLM adjutant replies `HOLD` for quiet situations, and the game reacted with `ClearSquadWaypoints()` — erasing the move waypoint the CO just placed via dashboard, so the squad stopped mid-move every cycle. Fix: `ExecuteWaypoint(sAction, vPos, bUserOrder)` marks CO-placed waypoints (`/orders` from dashboard/voice, incl. follow/medic); LLM HOLD only clears waypoints when `!HasUserWaypoint()`. Explicit CO `hold` cmd still clears everything (CO outranks adjutant).
...
78. **D.3 pending_orders is durable state, not ephemeral**: the game drains `/orders` every 2s, but a bridge restart mid-queue silently dropped orders (e.g. a CO-approved formation or voice order lost forever). Every queue mutation (append in POST /orders, tools, voice, suggestion approval; pop in GET /orders) now goes through `_persist_pending_orders()` → `pending_orders.json` (gitignored), restored in `startup_event()` via `_load_pending_orders()` with corrupt/non-list guards. Lesson: any queue that crosses a process boundary should survive a restart.
79. **D.4 watchdog: no restarts below the failure threshold**: `bridge_watchdog.ps1` polls /health and restarts the bridge only after FailLimit (3) CONSECUTIVE failures — a single dropped HTTP request (client timeout, LLM proxy hiccup) must never kill a healthy bridge. `-Once` mode exits with the failure count for testing. Verified: healthy → exit 0; dead port → exit 1 + no restart.
82. **E.2 config-driven tuning**: bridge-side magic numbers now live in config.json — `memory` block (`soldier_retention_days`, `event_window`, `thought_window`, `conversation_exchanges`, `battle_log_size`) + `game.session_gap_seconds` — read once at import (they already had `game.death_grace_cycles`). Game-side numbers (thought cooldowns, health thresholds) stay in the `.c` files; don't move those (the game can't read config.json).
80. **`powershell -File` cannot run .bat files**: the loop prompt's sync command `powershell -NoProfile -ExecutionPolicy Bypass -File scripts\sync-agent-docs.bat` FAILS with "file does not have a '.ps1' extension" — syncs that look like they ran (banner + no error) did nothing. Run via `cmd /c scripts\sync-agent-docs.bat` or copy AGENTS.md to CLAUDE.md/.goosehints/.github/copilot-instructions.md directly. Also: `.github/copilot-instructions.md` frequently lags behind the other copies because every agent runs its own sync with different tooling — check all three before committing docs.
81. **E.1: verify requirements.txt against a REAL fresh install**: the file looked complete on inspection (all deps present in the running venv), but the only proof that counts is a scratch `python -m venv` + `pip install -r requirements.txt` + import + endpoint smoke test. Took ~2 min and validated the bridge boots with zero manual steps. Rule 42's "deps were missing once" is exactly the failure mode this catches — do it after ANY dependency change, cheap insurance.
83. **C.3 voice overrides: index vs voice-id duality**: `assign_voice` accepts an int (EDGE_VOICES index), a voice-id string (resolved via `EDGE_VOICES.index()`), or None/"default" to remove. Validation happens at assign time (out-of-range/non-numeric rejected → False), and `_load_voice_assignments` re-validates indices on load — a hand-edited assignments file can never produce an IndexError at speak time. `_resolve_voice_index(member_name, member_index)` centralizes the override-wins-else-index rule so BOTH engines (edge + pyttsx3) honor it. Also: `EDGE_VOICES` is module-level in tts_handler.py, NOT an instance attribute — main.py must import it explicitly (`from tts_handler import TTSHandler, EDGE_VOICES`), `tts_handler.EDGE_VOICES` raises AttributeError.
78. **C.2 gating logic must be a pure function**: the chatter "should I speak now?" decision (`_chatter_due`) returns `(due, reason)` with zero side effects — every rule (tts disabled / no session / interval not elapsed / orders pending / recent battle) is unit-testable in isolation, and the asyncio loop stays a dumb driver. Battle-log ages are parsed from the `[HH:MM:SS]` prefix; midnight rollover makes the naive subtraction NEGATIVE — clamp to 3600s ("long ago"). Chatter is audio-only: writing it into soldier memory files would pollute conversation history with background noise.
79. **The overnight supervisor runs CONCURRENT agents (IMPORTANT)**: `scripts/dev_loop.ps1` (PID may differ per night) spawns a NEW `pi -p --no-session` agent every ~15 min until 08:19 — iterations overlap with YOUR session. Symptoms: files change under you mid-edit (edit tool rejects with "Could not find edits[N]"), commits appear that you didn't make (bat files, doc copies), and `git add -A` by the other agent sweeps YOUR uncommitted work into ITS commit (C.2 code landed inside the C.5 commit 8a070092 — code safe, message misleading). Defenses: (a) check `Get-CimInstance Win32_Process` for `cmd.exe ... pi -p --no-session` + `scripts/dev_loop.log` before starting; (b) re-read target regions IMMEDIATELY before each edit batch; (c) `git status`/`git log` before committing, stage EXPLICIT file lists, never `git add -A`; (d) if your edit batch fails atomically, the file changed — re-verify and re-issue; (e) expect your commit to be one of several stacked that night — push early and often.
85. **Model eval lessons (D.2)**: (a) phi4-style "reasoning" models are a trap for roleplay/JSON tasks — they leak chain-of-thought into the output and burn max_tokens; probe raw output before trusting a benchmark. (b) The proxy's 14B+ models are UNLOADABLE on its GPU (Ministral OOM, gemma4-12b-gpu1 bad config) — "better model" must be verified by actually loading, not by name. (c) Latency is the killer metric: llama3-8B produces valid JSON but at 27-72s/thought vs llama3.2-3b's 0.49s — a 50-150x gap that no quality gain justifies for the live 2-10s SITREP cycle. (d) CONCURRENT eval runs fight over the proxy's single GPU: the dev-loop agent and I both ran model_eval.py at once — each model load evicts the other, requests time out (120s+), and results are misleading. Check `Get-CimInstance Win32_Process` for an existing `model_eval.py` before starting one (rule 79 family). (e) ALWAYS persist baseline results to disk — my 6-sample llama3.2-3b run was console-only and nearly lost when the run timed out before writing. **UPDATE (c) by rule 86**: the "50-150x slower" llama3 latency was measured under concurrent eviction — clean resident measurements show parity (~0.5s); quality favors llama3, verdict reversed, config switched to llama3 (8B).
86. **Contradictory eval numbers across concurrent sessions = proxy eviction, not physics (D.2)**: two independent D.2 sessions reached OPPOSITE latency verdicts (agent A: llama3.2-3b 0.49s vs llama3 27-72s; agent B: llama3 0.46s vs llama3.2-3b 21.3s) — each run was corrupted by the other loading/evicting models on the proxy's single GPU (rule 85d). The quality metrics agreed in ALL 3 runs (llama3: valid 1.0, tool 0.25, parrot 0 — 12/12 samples; llama3.2-3b: valid 0.875-1.0, tool 0.125-0.167). Reconciliation: only RESIDENT-model latency is meaningful (~0.5s for both); the model the bridge uses stays resident in steady state (2-10s SITREP cycle) so latency is parity and QUALITY decides → switched to llama3 (8B). When eval results contradict: distrust latency first, reconcile on quality, check for concurrent processes (rule 79), never trust a verdict based on the other session's "slow" model.
84. **/status must not dump raw app_state (FIXED)**: C.2 added `chatter_task` (an asyncio.Task) to app_state — GET /status returned the raw dict, `jsonable_encoder` crashed on the Task and the endpoint 500'd (live-verified). The dashboard's `pollStatus()` swallows fetch errors, so battle log / AI thoughts / squad cards / tactical map all silently died. Fix: `receive_status_get` builds a JSON-safe view — skips `chatter_task` AND drops any value that fails encoding (future-proof). Same drift-guard lesson for `/soldiers`: its relationships comprehension lacked the `isinstance(v, dict)` guard that `get_social_summary` has — a bare-string relationship in an old memory file would 500 /soldiers. Guarded + non-dict opinions filtered. Unit tests: `test_status_guard.py` (TestClient, no LLM, tmp dirs). Also: `test_client.py` gained `status`/`suggestions` smoke tests (pure GETs, no LLM) so the running-bridge suite covers the dashboard live view.
...
- **E.1 requirements.txt clean-install verified**: scratch venv (`python -m venv venv_test`) + `pip install -r requirements.txt` → bridge imports, ALL endpoints 200 via TestClient (`/health` `/tts` `/soldiers` `/dashboard` `/voice`), and all 3 unit-test suites (`test_chatter.py`, `test_fatigue.py`, `test_soldier_thoughts.py`) pass on the fresh env. No requirements.txt changes needed — every dep the code imports is pinned. Scratch venv removed after the test.
...
- **D.3 Orders backlog persistence**: `pending_orders` (the queue the game drains via GET /orders every 2s) now survives bridge restarts — every mutation persists to `pending_orders.json` (gitignored) via `_persist_pending_orders()`, restored in `startup_event()` via `_load_pending_orders()` (guards: corrupt file, non-list). Wired at all 5 mutation sites: POST /orders, GET /orders pop, soldier `call_medic` tool, CO suggestion approval, voice pipeline. Unit tests: persist/restore round-trip, corrupt backlog ignored, non-list ignored. Live: queued order → bridge restart → order delivered.
- **D.4 Bridge watchdog**: `scripts/bridge_watchdog.ps1` polls `/health` (default 30s interval) and restarts the bridge via `restart_bridge_console.ps1` after 3 consecutive failures; logs to `scripts/bridge_watchdog.log` (gitignored). `-Once` mode + `-Url`/`-FailLimit` params for testing. Verified: healthy bridge → exit 0; dead port → failure logged, exit 1, NO restart below threshold. D.3's backlog makes restarts safe (queued orders survive).
- **E.2 Config-driven tuning**: bridge-side magic numbers moved to config.json — `memory` block (soldier_retention_days=7, event_window=50, thought_window=10, conversation_exchanges=10, battle_log_size=15) + `game.session_gap_seconds=90` (joining `game.death_grace_cycles`). Read once at import into module constants (`SOLDIER_RETENTION_DAYS`, `EVENT_WINDOW`, `THOUGHT_WINDOW`, `CONVERSATION_EXCHANGES`, `BATTLE_LOG_SIZE`, `SESSION_GAP_SECONDS`). Unit test: battle-log cap honors the config value + defaults sanity. Game-side numbers (thought cooldowns, health thresholds) intentionally stay in `.c` files.
- **C.4 Commander map view**: dashboard "Tactical Map" panel (center column) — canvas mini-map with the squad at center and enemy contacts plotted from SITREP relative offsets (dx east/right, dz south/down, clamped to the range edge), distance labels, 100m range rings, north arrow, legend, zoom buttons (200/400/800m half-range). Purely client-side — no bridge changes, data comes from the existing `/status` poll. Map math verified headless in node (east/south/north-west offsets, clamping, labels); JS syntax-checked; served 200.
- **C.3 Per-soldier voice selection in dashboard**: soldier detail panel now has a Voice dropdown — "default" + the 10 edge-tts voices (from `/tts.voice_options`), current override preselected (`/tts.assignments`). Changing it POSTs `/voice_assign` `{name, voice}` (voice = index, voice-id string, or "default" to remove). Overrides live in `tts_handler.voice_overrides` (name → EDGE_VOICES index) persisted to `voice_assignments.json` (gitignored) — survive bridge restarts. `_resolve_voice_index(member_name, member_index)` makes the override win in BOTH engines (edge voice + pyttsx3 index); unknown names fall back to the fixed-by-index default. Unit tests: `test_voice_assign.py` — assign by index/id, invalid rejected, remove via None/"default", persistence across handler re-creation, status shape, endpoint assign/reject/require-name/remove. Live: POST /voice_assign assign + remove verified against the running bridge, /tts exposes voice_options+assignments.
- **C.2 Radio chatter layer**: during quiet periods (session active, no CONTACT/CRITICAL/DOWN battle event in last `quiet_seconds`, no pending orders) the squad randomly "talks" over the radio via TTS — 8 patrol lines from `CHATTER_LINES`, voiced by a random ALIVE squad member (`_chatter_member`, voice index = `_name_hash(name,"voice") % 10`, stable). Gating is a PURE function `_chatter_due(now) -> (due, reason)` — unit-testable; the `_chatter_loop` asyncio task (started in `startup_event`, task handle in `app_state["chatter_task"]`) re-arms on no-session so a fresh session doesn't chatter instantly. Battle-log event age parsed from the `[HH:MM:SS]` prefix (`_battle_event_age_seconds`, midnight-rollover clamped to 3600s = old). Budget: `max_per_session` (12) utterances, reset at session boundary; interval `min/max_interval_seconds` (45-150) in config `chatter` block. Deliberately NOT written to soldier memory files (audio ambience ≠ conversation). `/tts` exposes `chatter` status (enabled/count/last/last_member/last_age); dashboard header shows it via /health fields. Unit tests: `test_chatter.py` — all 6 gating reasons, event-age parsing, dead-member exclusion, name formatting, voice-index stability. LIVE VERIFIED: chatter #1 fired 54s after restart (Alpha_2: "copy all. Radio discipline, everyone.") during a real quiet window.
- **D.2 Better LLM model — switched to llama3 (8B)** (see git log for commit): `scripts/model_eval.py` benchmarks models on the LAN proxy with the bridge's OWN prompt (`AI_THOUGHT_SYSTEM_PROMPT_SOLO`) + parser (`extract_json_block`): valid-JSON rate, tool rate, mood validity, parrot rate, latency. THREE independent runs across 2 sessions (`scripts/model_eval_results.json` — merged artifact: 8/8/8 samples for llama3.2-3b/llama3/laguna-s-2.1 + failure rows for Ministral/phi4/gemma4-12b-gpu1). Quality AGREED in every run: **llama3: valid 1.0, tool 0.25, parrot 0 (12/12 samples)**; llama3.2-3b: valid 0.875-1.0, tool 0.125-0.167 (1 schema failure in 14); laguna-s-2.1 (9B): best raw quality (tool 0.5, radio-prose thoughts) but 211s avg — unusable real-time. Latency verdicts CONTRADICTED between sessions (mutual eviction corruption, rules 85d/86); clean resident measurements: ~0.5s for both. Verdict: llama3 (8B) is a quality upgrade at latency parity when resident (the configured model stays resident in the 2-10s SITREP cycle) → `config.json` switched llama3.2-3b → llama3, bridge restarted, `/health` shows `model: llama3`. Disqualified: Ministral-3-14B (CUDA OOM), gemma4-12b variants (proxy config), phi4/Phi-4-reasoning-plus (chain-of-thought leak into reply, 0.0 valid even at 2000 max_tokens), Qwen3-30B-A3B-Thinking (empty content, 292s/call). Re-run: `python_bridge\venv\Scripts\python.exe scripts\model_eval.py --models <names> --samples N`. V.1 live session should confirm llama3 in play (revert = one config line).
- **/status 500 regression (FIXED, rule 84)**: C.2's `chatter_task` (an asyncio.Task in app_state) made GET /status return 500 — `jsonable_encoder` cannot serialize Tasks, and the dashboard's `pollStatus()` silently swallowed the error, killing the battle log / AI thoughts / squad cards / tactical map views. `receive_status_get` now builds a JSON-safe state view (skips `chatter_task`, drops any value that fails encoding). Also hardened `/soldiers`: relationships comprehension now has the `isinstance(v, dict)` drift guard (parity with `get_social_summary`) and non-dict opinions are filtered — a bare-string relationship in an old memory file would have 500'd the roster. Unit tests: `test_status_guard.py` (12 checks via TestClient, tmp dirs, no LLM). Live verified: /status 200 with 37 state keys + all other endpoints 200 after bridge restart.
...
*C.2–C.5 are done (see Completed). Forward path:*
...
*D.3 and D.4 are done (see Completed). Forward path:*
...
- **D.2 Better LLM model (DONE — switched to llama3 (8B); evidence + verdict in Completed)**: llama3.2-3b → llama3. Re-run: `python_bridge\venv\Scripts\python.exe scripts\model_eval.py --models <names> --samples N` (aliases: llama3.2-3b/llama3/laguna-s-2.1/ministral/phi4/gemma4-12b-gpu1).

#### Phase E — HYGIENE

*E.1, E.2 and E.3 are done (see Completed).*
